...
def convolution(r, qmax, q0, Ndots_norm, sigma1, sigma2, gapi):
    ...
    # Предварительная обработка параметров сигм и зазора
    sigma1 = sigma1 * 0.4191  # Масштабирование первой сигмы
    sigma2 = sigma2 * 0.4191  # Масштабирование второй сигмы
    ...
    weights = torch.exp(-0.5 * q_window.pow(2)) / math.sqrt(2*math.pi)
    weights = weights.repeat(q0.size(0), 1)  # Расширение для всех точек
    
    # Блок вычислений без отслеживания градиентов
    with torch.no_grad():
...
def resolution_function_smooth_step(qmax_rel, q_rel, sigma1, sigma2, gap):
    ...
    transition_end = 0.5 + gap/2    # Все операции остаются на GPU
