...
def calculate_matrices_and_reflection(matr, rough_res, kmax, q, Ndots, gap):
    ...
    dm[..., 1, 0] = q_values
    dm[..., 1, 1] = -q_values
    dmt = dm[1:len(ro)-1, :,:,:].flip(0).clone()
    ...
    pm[..., 0, 0] = exp_neg
    pm[..., 1, 1] = exp_pos
    pmt = pm[1:len(ro)-1, :,:,:].flip(0).clone()

    # Вычисление обратных матриц
    dmi = torch.linalg.inv(dm)
    dmti = torch.linalg.inv(dmt)

    einsum_fn = partial(torch.einsum, 'nij,njk->nik')
    ...
    if len(ro) == 2:
        ...
        P = torch.einsum('mnij,mnjk,mnkl->mnil', dmt, pmt, dmti)
        Pe = reduce(einsum_fn, torch.unbind(P, dim=0))
        M = dmi[0] @ Pe @ dm[-1]
