...
def twin_plotter(matr, q, r, var_sigma1, var_sigma2, I0, var_Ibkg, var_I):
    ...
    # Только локальные КОПИИ без градиентов, для графика
    q_plot = q.detach().clone().cpu() * 1e-10
    r_scaled = (r * I0).detach().clone().cpu()
    r_conv_plot = (var_I * r_conv2).detach().clone().cpu()
    
    fig, ax = plt.subplots(figsize=(15, 2.6))
    
    # Графики — все на torch -> numpy (только локально!)
    ax.fill_between(q_plot.numpy(), r_scaled.numpy(), r_scaled.numpy(), color='purple', alpha=0.5)
    ax.plot(q_plot[:len(r_conv_plot)].numpy(), r_conv_plot.numpy())
