...
class varbounds:
    def __init__(self, bounds_tensor, loss_func, norm_coef, matrixx, sigma1, sigma2, I0, Ibkg, ref_type='model'):
        ...
        if self.ref_type == 'model':
            ...
            matrix = matrixx
            with torch.no_grad():
                self.coef_normalization = norm_coef*loss_func(matrix, sigma1, sigma2, I0, Ibkg, ref_type = ref_type)/torch.sum(torch.abs((matrix[0:-2, 1] + matrix[2:, 1] - 2 * matrix[1:-1, 1]) / pow((matrix[0:-2, 0] + matrix[2:, 0]), 2)))
        #print(matrix) как будто бы норм
    ...
    def objective_function(self, var_vector_d, var_vector_rho, var_sigma1, var_sigma2, var_I0, var_Ibkg):
        if self.ref_type == 'model':
            ...
            #print(var_vector_d, var_vector_rho)
        ...
        #print(result) #!=matrix!!!
        final_result = self.loss(viewed_result, var_sigma1, var_sigma2, var_I0, var_Ibkg, self.ref_type)
